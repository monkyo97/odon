...
export const Tooth: React.FC<Props> = ({ number, conditions, selected, onClick }) => {
  // toma el primero para “estado principal” visual
  const color = COLORS_BY_CONDITION[conditions[0]?.condition || ''] || 'transparent';

  return (
    <div
      onClick={onClick}
      className={`cursor-pointer relative flex flex-col items-center justify-center w-10 h-14 rounded-md border transition-all duration-150 ${
        selected ? 'border-blue-500 ring-2 ring-blue-400 bg-blue-50' : 'border-gray-300 hover:bg-gray-100'
      }`}
    >
      {/* SVG simplificado */}
      <svg width="24" height="26" viewBox="0 0 24 26" className="text-gray-400">
        ...
        />
        {/* bloque de color para indicar “estado” */}
